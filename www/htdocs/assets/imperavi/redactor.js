	...
	Construct.prototype = {
		...
		*/				
        ...
        imageSave: function(el)
        {
            ...
            var style = '';
            if ($('#redactor_form_image_align').val() != 0)
            {
                var floating = $('#redactor_form_image_align').val();
                
                if (floating == 'left') $(el).css({ 'float': 'left', margin: '10px' });
                else if (floating == 'right') $(el).css({ 'float': 'right', margin: '10px' });
            }
            else $(el).css({ 'float': 'none', margin: '10px auto' });
        ...
        { 
					redactorActive = this; // todo жестоко вклинился
        	redactorActive._imageSet(data);
        },
